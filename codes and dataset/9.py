...
def calculate_cost_test(acts,ydata,data_norm_y,data_norm_x):
...
def calculate_cost(acts,ydata,data_norm_y,data_norm_x,weights):
...
def update_param(weights,biases,dw,db,learning_rate,lamda,mom_flag,dw_m,dw_v,beta1,beta2,db_m,db_v,t):
    # print("hello 0")
    if mom_flag == 0 :
        # print("hello 1")
        dw_m = []
        ...
        for i in range(len(weights)) :
            ...
            dw_m[i] = beta1*(dw_m[i]) + (1-beta1)*dw[i]
            dw_v[i] = beta2*(dw_v[i]) + (1-beta2)*np.power(dw[i],2)
            mcap_w = (dw_m[i])/(1-np.power(beta1,t))
            vcap_w = (dw_v[i])/(1-np.power(beta2,t))
            # print("weights before adam - ",weights[i].shape)
            weights[i] = weights[i] - learning_rate*mcap_w/np.power(vcap_w+0.00000001,1/2) 
            # print("weights after adam - ",weights[i].shape)

            db_m[i] = beta1*(db_m[i]) + (1-beta1)*db[i]
            db_v[i] = beta2*(db_v[i]) + (1-beta2)*np.power(db[i],2)
            mcap_b = (db_m[i])/(1-np.power(beta1,t))
            vcap_b = (db_v[i])/(1-np.power(beta2,t))
            # print(mcap_b.shape)
            # print("biases before adam - ",biases[i].shape)
            biases[i] = biases[i] - learning_rate*mcap_b/np.power(vcap_b+0.00000001,1/2)
            # print("biases after adam - ",biases[i].shape)
            # #added weights cap
            #  for j in range(len(weights[i])):
            #      for k in range(len(weights[i][j])):
            #          if weights[i][j][k]>1:
    ...
    return weights,biases,dw_m,dw_v,db_m,db_v,t
...
for i in range(e) :
    ...
    for batch_num in range(num_of_batches):
        ...
        weights,biases,dw_m,dw_v,db_m,db_v,t = update_param(weights,biases,dws,dbs,learning_rate,lamda,mom_flag,dw_m,dw_v,beta1,beta2,db_m,db_v,t)
        mom_flag = 1
...
plt.show()
# with open('g.8.b.cost_convergence.pkl','rb') as f:
#     x = pickle.load(f)
#     print(x.shape)

# plt.plot(iter_map,x,"red")
# plt.plot(iter_map,costs_map,"green")
# plt.gca().legend(( 'mom', 'adam'))
# plt.show()
# cost_convergence = np.array(costs_map)
# with open('g.9.b.cost_convergence.pkl','wb') as f:
