...
def calculate_cost_test(acts,ydata,data_norm_y,data_norm_x):#data for denormalizing
    ...
    # cost = (1/m)*((yp-y)/y)
    # cost = (1/m)*np.sum((yp-y)/y)
    # cost = np.sum((yp[0]-y[0])/y[0])
    return cost
...
def update_param(weights,biases,dw,db,learning_rate,lamda,mom_flag,dv,beta,db_mom):
    if mom_flag == 0 :
        dv = []
        db_mom = []
        ...
        for i in range(len(weights)) :
            # print("weights[i] shape - ",weights[i].shape)
            # weights[i] = weights[i] - learning_rate*dw[i] - lamda*(np.absolute(weights[i]))
            dv[i] = beta*(dv[i]) + (1-beta)*dw[i]
            weights[i] = weights[i] - learning_rate*dv[i] #- lamda*weights[i]
            db_mom[i] = beta*(db_mom[i]) + (1-beta)*db[i]
            biases[i] = biases[i] - learning_rate*db_mom[i] 
            # #added weights cap
            #  for j in range(len(weights[i])):
            #      for k in range(len(weights[i][j])):
            #          if weights[i][j][k]>1:
    ...
    return weights,biases,dv,db_mom
...
for i in range(e) :
    ...
    for batch_num in range(num_of_batches):
        ...
        weights,biases,dv,db_mom = update_param(weights,biases,dws,dbs,learning_rate,lamda,mom_flag,dv,beta,db_mom)
        mom_flag = 1
...
plt.plot(iter_map,x,"green")
plt.plot(iter_map,costs_map,"blue")
plt.gca().legend(( 'cap without momentum', 'with Momentum'))
plt.show()
cost_convergence = np.array(costs_map)
with open('8.cost_convergence.pkl','wb') as f:
