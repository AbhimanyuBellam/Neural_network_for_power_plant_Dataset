...
plt.show()
with open('g.5.b.cost_convergence.pkl','rb') as f:
    x = pickle.load(f)
    print(x.shape)

plt.plot(iter_map,x,"black")
plt.plot(iter_map,costs_map,"blue")
plt.gca().legend(( 'without cap', 'with cap'))
plt.show()
cost_convergence = np.array(costs_map)
with open('g.6.b.cost_convergence_cap.pkl','wb') as f:
