...
for i in range(e) :
    ...
    for batch_num in range(num_of_batches):
        ...
        # print(xdata_full.shape)
        # print(ydata_full.shape)
        # print(nx_data.shape)
        # print(ny_data.shape)
...
with open('4.2.cost_convergence.pkl','rb') as f:
    x = pickle.load(f)
    print(x.shape)
plt.plot(iter_map,x,"red")
plt.plot(iter_map,costs_map,"green")
plt.gca().legend(( 'no I/O', 'I/O'))
plt.show()
cost_convergence = np.array(costs_map)
with open('5.cost_convergence.pkl','wb') as f:
