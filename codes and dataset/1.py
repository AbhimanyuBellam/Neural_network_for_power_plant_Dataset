...
with open('weights.pkl','wb') as f:
    pickle.dump(npWeights, f)
npBiases = np.array(biases)
with open('biases.pkl','wb') as f:
    pickle.dump(npBiases, f)

x_val = generate_validation_data(300)
yp = predict(x_val,weights,biases,tanh)
x = np.reshape(xdata,(1000,1))
y = np.reshape(ydata,(1000,1))
x_val = np.reshape(x_val,(300,1))
yp = np.reshape(yp,(300,1))
plt.plot(x,y)
plt.plot(x_val,yp)
plt.show()
